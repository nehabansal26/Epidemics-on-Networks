...
def process_network(G, beta, initial_I, cnt_G):
    ...
    sim = EoN.Gillespie_SIR(G=G, tau=beta, gamma=1/7, initial_infecteds=infected_nodes,
...
def main(filename, st, end):
    ...
    with open(f"{filename}_{st}_{end}_7_SIR.pkl", 'wb') as f:
        pickle.dump(SIR_dict, f)
    print(f"{filename} done!")

    result = subprocess.run(f"scp /home/neha/{filename}_{st}_{end}_7_SIR.pkl neha@siren.maths.cf.ac.uk:/home/neha/results/"
    ...
    os.remove(f"/home/neha/{filename}_{st}_{end}_7_SIR.pkl")
