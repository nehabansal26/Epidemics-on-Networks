...
parser.add_argument('join_type', type=str, help='joining sample and population df')
parser.add_argument('dest_dir',type=str,help='location for saving results')
args = parser.parse_args()

def transfer_read_del_file(network_filename,file_type):
...
sir_files =  transfer_read_del_file(args.network_filename,'SIR')
...
write_transfer_del_file(OG_df,args.network_filename,"OG_SIR_agg",args.dest_dir)

sample_dict = transfer_read_del_file(args.network_filename,'samples')
for key, value in sample_dict.items():
    print(f"{key} starts")
    df_ls = [create_sample_df(net_idx
                              ,key
                              ,sample_array,sir_df[sir_df['net_idx']==net_idx]
                              ,rm_dup=args.dup 
    ...
    write_transfer_del_file(df,args.network_filename,f"{key}_SIR_agg",args.dest_dir)
